...
export const Scene3: React.FC<{ f: number }> = ({ f }) => {
  const c = sceneCam(f, 300, 450, 10, -12);
  ...
  const custT = (f % 60) / 60;
  const breathe = (f % 48) / 48;
  const haze = Math.sin(f / 140) * 22;
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: COLOR.black }}>
      <div style={{ position: "absolute", inset: 0, transform: cam, transformOrigin: "center center" }}>
        ...
        <AtmosphericHaze drift={haze} y={30} />
        <LeftScrim width={492} feather={130} top={110} height={1000} />
        <SoftMatte x={470} y={230} w={220} h={290} />
        <StoreGlow x={S3_CROWN_GLOW.x} y={S3_CROWN_GLOW.y} r={S3_CROWN_GLOW.r} strength={0.03 + (0.5 + 0.5 * Math.sin(f / 34)) * 0.015} />
        ...
        <svg width={1080} height={1920} viewBox="0 0 1080 1920" style={{ position: "absolute", inset: 0 }}>
          ...
          <RouteOverlay d={S3_MAIN_ROUTE} reveal={mainReveal} pulse={mainPulse} width={5} dying={0.35} breathe={breathe} />
          {/* competitor branch: successful connection */}
          <RouteOverlay d={S3_COMP_ROUTE} reveal={compReveal} pulse={compPulse} width={5} breathe={breathe} />
          {compReveal > 0.9 && (
            <>
